...
class CataloguerService:
        ...
        return self.remove_empty_fields(
            {
                ...
                "capability_type": st.selectbox(
                    "Tipo",
                    ["sensor", "actuator"],
                    help="Sensor: sente o ambiente; Actuador: modifica o ambiente",
                    disabled=edit,
                ),
                "description": st.text_input(
                    "Descrição", placeholder="Mede a temperatura da sala"
                ),
            }
        )
    ...
    def resource_fields(self):
        ...
        with col1:
            description = st.text_input(
                "Descrição",
                placeholder="Estacionamento em São Luís",
                help="Uma descrição curta do recurso",
            )
            lat = st.number_input("Latitude", format="%.6f")
            collect_interval = st.number_input(
                "Intervalo de Coleta (segundos)",
                min_value=0,
                help="Valor numérico, em segundos, que corresponde ao intervalo de coleta de dados",
            )
        ...
        with col2:
            ...
            uri = st.text_input(
                "URI", placeholder="exemplo.com", help="Unified Resource Identifier"
            )

        return {
            "data": self.remove_empty_fields(
                {
                    "uri": uri,
                    "lat": lat,
                    "lon": lon,
    ...
    def search_fields(self):
        ...
        with col1:
            capability = st.multiselect(
                "Capacidades", self.lista_capacidades, placeholder="Escolha uma opção"
            )
            lat = st.number_input("Latitude", format="%.6f")
            postal_code = st.text_input(
                "Código Postal", help="Formato: XXXXX-XXX", placeholder="XXXXX-XXX"
            )
