...
class CollectorService:
        ...
        if not last:
            ...
            with col1:
                start_day = st.date_input(
                    "Início do intervalo", format="YYYY-MM-DD", value=None
                )
                start_hour = st.time_input(
                    "aa", label_visibility="collapsed", value=None
                )

            with col2:
                end_day = st.date_input(
                    "Fim do intervalo", format="YYYY-MM-DD", value=None
                )
                end_hour = st.time_input(
                    "aas", label_visibility="collapsed", value=None
                )

            if start_date and end_date and start_hour and end_hour:
                start_date, end_date = (
                    f"{start_day}T{start_hour}",
                    f"{end_day}T{end_hour}",
                )

        uuids = st.multiselect(
            "Recursos (uuids)",
            self.lista_recursos,
    ...
    def form(self, key: str):
        method = "POST"
        base_endpoint = "/collector/resources/data"
