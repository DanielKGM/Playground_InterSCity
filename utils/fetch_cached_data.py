...
@cache_data(show_spinner="Buscando dados...", max_entries=1)
def fetch_capabilities_from_resource(
    uuid: str, base_url: str = get_base_url()
) -> list[str]:
...
def fetch_all_resources(base_url: str = get_base_url()) -> dict | None:
    ...
    if "resources" in content:
        ...
        for resource in resources:
            r_uuid = resource.get("uuid")
            r_desc = resource.get("description") or f"Sem descrição ({r_uuid})"
...
@cache_data(
    ...
    max_entries=1,
    hash_funcs={
        dict: lambda d: json.dumps(
            d, sort_keys=True
        )  # Serializa dicionários para uma string ordenada
    },
)
def request(
    method: str,
